...
prices <- bind_rows(pricesCHF, pricesEUR_CHF) %>% 
  arrange(symbol, date) %>% 
  group_by(symbol) %>%
  mutate(close_chf = na.locf(close_chf)) %>% 
  ungroup()

dividends <- bind_rows(dividendsCHF, dividendsEUR_CHF)
...
price_returns %>% 
  ...
  coord_flip() +
  labs(title = "Price returns of selected stocks/ETFs", subtitle = "since Jan 2016, monthly returns, arithmetic, excl. dividends",
       x = "Stock", y = "return") +
  theme_tq() +
...
portfolio_returns %>% mutate(symbol = "Portfolio") %>% 
  ...
  scale_y_continuous(labels = scales::percent) +
  coord_flip() +
  labs(title = "Price returns on equal weightes stocks/ETFs portfolio", subtitle = "since Jan 2016, monthly returns, arithmetic, excl. dividends",
       x = "", y = "return") +
  theme_tq() +
  theme(legend.position = "none")
```
...
data <- read_csv2("data/temp.csv", skip = 1)
...
bind_rows(portfolio_value %>% mutate(return = "Portfolio"),
  ...
  labs(title = "Portfolio performance over time",
       subtitle = "since Jan 2016, monthly returns, arithmetic, excl. dividends",
...
prices %>% 
  bind_rows(portfolio_value %>% mutate(close_chf = wealth, symbol = "Portfolio")) %>% 
  filter(date >= as.Date("2016-03-31")) %>% 
  group_by(symbol) %>% 
  mutate(return = close_chf/first(close_chf) - 1) %>% 
  ggplot(aes(x = date, y = return, color = symbol)) +
  geom_line(aes(alpha = symbol == "Portfolio")) + 
...
price_returns %>% filter(date > date("2016-03-01")) %>%
  ...
  labs(title = "Risk return diagram", subtitle = "since Mar 2016, annualized values, portfolio equal weighted and monthly rebalanced",
...
dividends_paid %>% 
  ...
  scale_x_continuous(limits = c(2016, 2020.6)) +
